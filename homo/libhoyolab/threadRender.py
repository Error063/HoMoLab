...
def replaceAll(contents: str, emotionDict: dict):
    ...
    contents = re.sub(replace_regex.emotion,
                      ...
                      contents)
    contents = re.sub(replace_regex.article, lambda m: f'<a href="/article?id={m.group(1)}" class', contents)
    contents = re.sub(replace_regex.user, lambda m: f'<a href="/user?uid={m.group(1)}" class', contents)
    return contents
